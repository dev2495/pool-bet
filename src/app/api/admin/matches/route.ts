...
// POST /api/admin/matches — create a match with its outcomes (allowed before the session is closed).
export async function POST(req: NextRequest) {
  return handle(async () => {
    ...
    if (s.status !== "DRAFT" && s.status !== "OPEN" && s.status !== "LIVE")
      return err("Cannot add matches once session is CLOSED/SETTLED", 400);

    // Initial match status mirrors session state.
    const initialStatus = s.status === "OPEN" || s.status === "LIVE" ? "OPEN" : "PENDING";
